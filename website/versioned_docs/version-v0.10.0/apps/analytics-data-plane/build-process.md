...
This repository is setup as a monorepo with pnpm. The main components are in the `apps` and `libs` folders. They are linked together in one monorepo by pnpm, the file where this is specified is called `pnpm-workspace.yaml`. The `apps` folder contains the analytics-data-plane-ui and analytics-data-plane-api packages, the `libs` folder contains the `dto`s that are common between the analytics-data-plane-ui and the analytics-data-plane-api. At the root of the monorepo we define a root `package.json` and a `Dockerfile`. This means that there is one docker image for both the analytics-data-plane-api and the analytics-data-plane-ui. The analytics-data-plane-ui can be enabled in the analytics-data-plane-api by setting an environment variable called `EMBEDDED_FRONTEND`.
...
And afterwards you can run a command by using `pnpm ...` with any command that is listed in the `package.json` file. If you want to run a command only for a specific package (for example analytics-data-plane-api) you can run `pmpm --filter analytics-data-plane-api ...` where at the dots you can place a command that is in the `package.json` of that specific app/lib.
...
To run the analytics-data-plane-ui and analytics-data-plane-api in watch mode use the following command:

```
pnpm run dev:analytics-data-plane-api
```

and

```
pnpm run dev:analytics-data-plane-ui
```
...
If you want to test interactions between control planes excecute the following commands to generate a second instance. For this you would need to create a separate config file (based on the existing default config at `./apps/analytics-data-plane-api/config.yaml`) as well as provide the analytics-data-plane-api used by the analytics-data-plane-ui via the `BACKEND` environment variable:

```
CONFIG_PATH=$(pwd)/apps/analytics-data-plane-api/config_second.yaml pnpm dev:analytics-data-plane-api
```

```
BACKEND=http://localhost:3002 pnpm dev:analytics-data-plane-ui
```
...
```
docker build -t analytics-data-plane .
```

Then run it:

```
docker run -p 3000:3000 analytics-data-plane
```

And visit http://localhost:3000 to view the analytics data plane.
...
```
pnpm --filter analytics-data-plane-api migrations:generate
```
...
```
pnpm --filter analytics-data-plane-api migrations:check
```
