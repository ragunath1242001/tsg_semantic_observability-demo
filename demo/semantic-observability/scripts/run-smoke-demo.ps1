param(
  ...
  [string]$LocalParticipant = "did:web:localhost%3A3501",
...
$datasetId = "urn:tsg:docker-demo:dataset:$runId"
$offerId = "urn:tsg:docker-demo:offer:$runId"
$agreementId = "urn:tsg:docker-demo:agreement:$runId"
$transferId = "urn:tsg:docker-demo:transfer:$runId"
...
$datasetConfig = @{
  type = "versioned"
  title = "Docker Demo Dataset $runId"
  description = @(
    "Synthetic dataset used to demonstrate privacy-safe semantic observability output."
  )
  baseSemanticModelRef = "https://example.org/semantic/docker-demo/base"
  currentVersion = "1.0.0"
  versions = @(
    @{
      version = "1.0.0"
      semanticModelRef = "https://example.org/semantic/docker-demo/1.0.0"
      distributions = @(
        @{
          backendUrl = "https://mockhttp.org/anything"
          mediaType = "application/json"
          schemaRef = "https://example.org/schemas/docker-demo-1.0.0.json"
...
$catalogDataset = @{
  "@type" = "Dataset"
  "@id" = $datasetId
  title = "Docker Demo Catalog Dataset $runId"
  description = @(
    "Catalog metadata created by the semantic observability Docker demo."
  )
  conformsTo = @("https://example.org/semantic/docker-demo/1.0.0")
  version = "1.0.0"
  hasPolicy = @(
    @{
      "@type" = "Offer"
  ...
  distribution = @(
    @{
      ...
      conformsTo = @("https://example.org/schemas/docker-demo-1.0.0.json")
      format = "HTTP"
      title = "Mock JSON API"
    }
  )
...
Write-Host "Control Plane API and UI: $ControlPlaneUrl"
Write-Host "HTTP Data Plane API and UI: $HttpDataPlaneUrl"
