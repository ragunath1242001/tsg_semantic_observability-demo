import { ExecutionContext, HttpStatus, Injectable, createParamDecorator } from "@nestjs/common";
...
export const VPId = createParamDecorator((_, context: ExecutionContext): string | undefined => {
  const request = context.switchToHttp().getRequest();
  if (!request.user) return undefined;
  const vp = plainToInstance(VerifiablePresentation, request.user);
...
@Injectable()
export class VerifiablePresentationStrategy extends PassportStrategy(Strategy, 'vp') {
  ...
  async validate(token: string) {
    ...
    if (!valid) {
      throw new DSPError('Verifiable Presentation token not valid', HttpStatus.UNAUTHORIZED);
    }

    if (!tokenPayload['vp']) {
      throw new DSPError('No "vp" field in token payload', HttpStatus.UNAUTHORIZED);
    }
