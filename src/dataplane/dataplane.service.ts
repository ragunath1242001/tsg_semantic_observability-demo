...
import {
  ...
  DataPlaneDetailsDto,
  DataPlaneAddressDto,
} from "../model/data-planes/dataPlanes.dto";
import {
  Dataset,
  Distribution,
  DataService,
...
export class DataPlaneService {
  ...
  async init() {
    setTimeout(async () => {
      ...
      const dataset = new Dataset({
        ...
        distribution: this.config.dataset.distributions.map(
          (distributionConfig) => {
            return new Distribution({
              id: distributionConfig.id,
              format: "dspace:HTTP",
              title: `Version ${distributionConfig.version}`,
              conformsTo: distributionConfig.openApiSpec
                ? new Reference(distributionConfig.openApiSpec)
                : undefined,
  ...
  async transferStart(
    ...
    const transfer = this.transfers.find(
      (transfer) => transfer.id === processId,
    );
  ...
  async executeProxyRequest(
    ...
    try {
      ...
      headers["authorization"] = transfer.dataAddress.endpointProperties.find(
