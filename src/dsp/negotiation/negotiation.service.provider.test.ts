...
import { ContractAgreementVerificationMessageDto, ContractNegotiationState, NegotiationEvent, ProofTypes } from "../../model/dsp/negotiation/messages.dto";
import { ContractAgreementVerificationMessage, ContractNegotiationEventMessage, ContractNegotiationTerminationMessage, ContractRequestMessage } from "../../model/dsp/negotiation/messages";
...
describe("Negotiation Service (Provider)", () => {
    ...
    beforeAll(async () => {
        ...
        server = setupServer(
            http.post<PathParams, ContractAgreementVerificationMessageDto>(`http://remoteparty.test/negotiation/${remoteProcessId}/offers`, () => {
                return HttpResponse.json({status: 'OK'})
            }),
            http.post<PathParams, ContractAgreementVerificationMessageDto>(`http://remoteparty.test/negotiation/${remoteProcessId}/agreement`, () => {
                return HttpResponse.json({status: 'OK'})
            }),
            http.post<PathParams, ContractAgreementVerificationMessageDto>(`http://remoteparty.test/negotiation/${remoteProcessId}/events`, () => {
                return HttpResponse.json({status: 'OK'})
            })
        );
    ...
    describe("Simple negotiation interactions", () => {
        ...
        it("Handle new negotiation request", async () => {
            ...
            const negotiation = await negotiationService.handleNewRequest(
                new ContractRequestMessage({
                    ...
                    dataSet: 'urn:uuid:b9e2af39-36a9-4e92-a02e-a05dcd94219b'
                }),
                'did:web:remoteparty.test'
            );
            expect(negotiation).toBeDefined();
            expect(negotiation.contractNegotiationState).toBe(ContractNegotiationState.REQUESTED);
            localProcessId = negotiation.processId;

            const negotiationDetail = await negotiationService.getNegotiation(localProcessId);
        ...
        it("Retrieve negotiation", async () => {
            ...
            const negotiationDetail2 = await negotiationService.getNegotiation(localProcessId, 'did:web:remoteparty.test');
            expect(negotiationDetail2).toBeDefined();
            await expect(negotiationService.getNegotiation(localProcessId, 'did:web:otherremoteparty.test')).rejects.toThrow('Cannot get negotiation with process ID');
            await expect(negotiationService.getNegotiation('urn:uuid:00000000-0000-0000-0000-000000000000')).rejects.toThrow('Cannot get negotiation with process ID');
        })
    ...
    describe("Complex negotiation interactions", () => {
        ...
        it("Handle new offer request", async () => {
            ...
            await expect(negotiationService.handleExistingRequest(
                localProcessId,
                new ContractRequestMessage({
                    processId: remoteProcessId,
                    offer: offer,
    ...
    describe("Negotiation termination", () => {
        ...
        it("Provider termination", async () => {
            ...
            const termination = await negotiationService.terminate(localProcessId, 'PROVIDER_ERROR', 'Request termination');
            expect(termination.status).toBe('OK');
        });
    });
})