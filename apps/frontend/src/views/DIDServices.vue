...
const loadServices = async () => {
  try {
    const response = await axiosInstance<DIDService[]>("management/did/services");
    services.value = response.data;
  } catch (err) {
    const message = err.response?.data?.message || "Could not load DID services";
    toast.add({
      severity: "warn",
      summary: "API error",
      detail: message,
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        ...
        const message = err.response?.data?.message || "Could not delete service";
        toast.add({
          severity: "warn",
          summary: "API error",
          detail: message,
      ...
      <template #content>
        <form @submit.prevent="addService">
          <FormField label="Service ID" v-slot="props">
            <InputText :id="props.id" class="w-full" v-model="serviceForm.id" :placeholder="`${store.state.user?.didId}#`" />
          </FormField>
