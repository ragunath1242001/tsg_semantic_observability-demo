...
const issuableContexts = computed(() => 
  config.value?.contexts?.filter(c => c.issuable)?.map(c => c.documentUrl) ?? []
)

const issuableCredentialTypes = computed(() => 
  config.value?.contexts?.filter(c => c.issuable)?.map(c => c.credentialType) ?? []
)
...
const issueCredential = async () => {
  ...
  try {
    ...
    toast.add({severity: 'success', summary: 'Credential issued', detail: `Credential ${credentialForm.value.id} successfully issued`, life: 10000});
    credentialForm.value = formDefault;
  } catch (err) {
    toast.add({severity: 'warn', summary: 'API error', detail: 'Could not issue credential', life: 10000});
  }

}

const useContext = (context: JsonLdContextConfig) => {
  credentialForm.value.context = [...new Set([...credentialForm.value.context, (context.documentUrl || '')])];
  credentialForm.value.schema = context.schema;
  if (!context.schema?.properties?.type && !context.schema?.properties?.['@type']) {
    credentialForm.value.type = [...new Set([...credentialForm.value.type, context.credentialType])]
  }
}

const updateCredentialSubject = () => {
  credentialForm.value.credentialSubject = JSON.stringify(credentialForm.value.credentialSubjectObject, null, 2);
}
...
const validateCredentialSubject = (showToast: boolean) => {
  try {
    ...
    const contexts: JsonLdContextConfig[] = config.value.contexts?.filter(c => credentialForm.value.type.includes(c.credentialType)) || []
...
<template>
  <div>
    <Card>
      ...
      <template #subtitle>Configured context in this wallet instance, which might be used for the issue process.</template>
      <template #content>
        <DataTable v-model:expanded-rows="expandedRows" :value="config?.contexts" paginator :rows="10">
          <Column expander style="width: 5rem" />
          <Column field="id" header="ID" />
          <Column field="credentialType" header="Credential Type" />
          <Column field="issuable" header="Issuable">
            <template #body="props">
              <i v-if="props.data.issuable" class="pi pi-check-circle text-green-500" />
              <i v-else class="pi pi-times-circle text-red-500" />
            </template>
          </Column>
          <Column field="schema" header="Schema">
            <template #body="props">
              <i v-if="props.data.schema" class="pi pi-check-circle text-green-500" />
              <i v-else class="pi pi-times-circle text-red-500" />
            </template>
          </Column>
          <Column field="actions" header="Actions">
            <template #body="props">
              <Button severity="success" label="Use" :disabled="props.data.default" @click="useContext(props.data)" />
            </template>
          </Column>
        </DataTable>
      </template>
    ...
    <Card class="mt-5">
      ...
      <template #subtitle>Issue a credential either a self-signed credential, or a credential for another party.</template>
      <template #content>
        <form @submit.prevent="issueCredential">
          <FormField label="Contexts" v-slot="props">
            <MultiSelect :id="props.id" class="w-full" v-model="credentialForm.context" :options="issuableContexts" placeholder="(Optionally) Add JSON-LD contexts" />
          </FormField>
          <FormField label="Type" v-slot="props">
            <MultiSelect :id="props.id" class="w-full" v-model="credentialForm.type" :options="issuableCredentialTypes" placeholder="Add a Credential type (only if not explicit in credential subject)" />
          </FormField>
          <FormField label="Target DID" v-slot="props">
            <InputText :id="props.id" class="w-full" v-model="credentialForm.targetDid" placeholder="did:web:..." pattern="did:web:.*" validation-message="Target DID must be a DID web" required />
          </FormField>
          <FormField label="ID" v-slot="props">
            <InputText :id="props.id" class="w-full" v-model="credentialForm.id" placeholder="ID" required />
          </FormField>
          <FormField label="Composite ID" v-slot="props">
            <InputText :id="props.id" class="w-full" :value="`${credentialForm.targetDid}#${encodeURIComponent(credentialForm.id)}`" disabled />
          </FormField>
          <FormField label="Credential" v-slot="props" v-if="!credentialForm.schema || credentialForm.manualCredential">
            <Textarea :id="props.id" class="w-full" style="font-family: monospace;" v-model="credentialForm.credentialSubject" rows="10" @blur="validateCredentialSubject(false)" />
            <Button severity="success" v-if="credentialForm.schema" label="Credential form" @click="credentialForm.manualCredential = false" />
          </FormField>
          <FormField label="Credential Form" :label-width="12" v-if="credentialForm.schema && !credentialForm.manualCredential">
            <JsonSchemaFormElement v-for="(child, key) in parsedProperties" :schema="child" :key="key" :required="credentialForm.schema.required.includes(key)" :name="key" :didId="didId" @input="($event) => {credentialForm.credentialSubjectObject[key] = $event; updateCredentialSubject();}"></JsonSchemaFormElement>
            <FormField no-label>
              <Button severity="warning" v-if="credentialForm.schema" label="Manual credential" @click="credentialForm.manualCredential = true" />
            </FormField>
          </FormField>
