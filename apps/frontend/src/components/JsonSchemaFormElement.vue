...
<template>
  <FormField :label="schema.title ?? name" class="pl-4" v-slot="props" :label-width="(typeRef === 'object') ? 12 : 2">
    <template v-if="typeRef === 'const'">
      <InputText v-model="valueRef" disabled class="w-full"/>
    </template>
    <template v-else-if="typeRef === 'enum'">
      <Dropdown :id="props.id" class="w-full" v-model="valueRef" :placeholder="name" :options="schema.enum" />
    </template>
    <template v-else-if="typeRef === 'string'">
      <InputText :id="props.id" class="w-full" v-model="valueRef" :placeholder="name" :required="required" :pattern="schema.pattern" :validation-message="(schema.pattern) ? `Field must conform to pattern: ${schema.pattern}` : undefined"/>
    </template>
    <template v-else-if="typeRef === 'number'">
      <InputNumber :id="props.id" class="w-full" v-model="valueRef" :placeholder="name" :required="required" />
    </template>
    <template v-else-if="typeRef === 'boolean'">
      <InputSwitch :id="props.id" class="w-full" v-model="valueRef" />
    </template>
    <template v-else-if="typeRef === 'object'">
      <JsonSchemaFormElement v-for="(child, key) in parsedProperties" :schema="child" :key="key" :name="key" :required="schema.required?.includes(key)" @input="emitValue"></JsonSchemaFormElement>
    </template>
