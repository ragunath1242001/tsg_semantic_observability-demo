...
import { AccessToken, CredentialIssuerMetadata, CredentialOffer, CredentialOfferRequest, CredentialOfferStatus, CredentialRequest, CredentialResponse, OfferGrants } from "@libs/dtos";
...
export class IssuerService {
  ...
  async revokeOffer(id: number): Promise<CredentialOfferStatus> {
    ...
    if (!issuance) {
      throw new AppError(`No credential issuance flow found for id ${id}`, HttpStatus.NOT_FOUND).andLog(this.logger);
    }
