...
export class ManagedIdentityWalletClient extends WalletClient {
  ...
  private async getWallet(): Promise<MiWWalletDetails> {
    if (!this.iamConfig.walletUrl) {
      throw new DSPError('No walletUrl configured for the Managed Identity Wallet', HttpStatus.INTERNAL_SERVER_ERROR);
    }
    try {
  ...
  async requestVerifiablePresentation(audience: string): Promise<VerifiablePresentationJwt> {
    ...
    const credential = walletDetails.verifiableCredentials.find(c => c.id === this.iamConfig.credentialId);
    try {
      const response = await axios.post<VerifiablePresentationJwt>(this.iamConfig.presentationUrl, {
