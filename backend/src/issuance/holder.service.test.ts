...
import { AccessToken, CredentialRequest, CredentialResponse, OfferGrants } from '../model/issuance.dto';
import { GenerateKeyPairResult, KeyLike, exportJWK, generateKeyPair } from 'jose';
...
describe("Holder service", () => {
  ...
  beforeAll(async () => {
    ...
    server = setupServer(
      ...
      http.get('http://localhost:3000/.well-known/openid-credential-issuer', async () => {
        return HttpResponse.json(await issuerService.issuerMetadata());
      }),
      http.post<PathParams, any, AccessToken>('http://localhost:3000/oid4vci/token', async (ctx) => {
        const data = await ctx.request.formData()
        return HttpResponse.json(await issuerService.createAccessToken(data.get('pre-authorized_code') as string))
      }),
      http.post<PathParams, CredentialRequest, CredentialResponse>('http://localhost:3000/oid4vci/credential', async (ctx) => {
        const body = await ctx.request.json();
        const authorization = ctx.request.headers.get('Authorization');
        return HttpResponse.json(await issuerService.handleCredentialRequest(authorization!.substring(7), body))
      })
    );
  ...
  describe("Issuance process", () => {
    it("Create offer", async () => {
      ...
      await holderService.requestCredential(offer.grants?.[OfferGrants.PRE_AUTHORIZATION_CODE]?.['pre-authorization_code']!, 'http://localhost:3000')

      const credentials = await moduleRef.get(CredentialsService).getCredentials();
