...
describe("Credentials Service", () => {
  ...
  beforeAll(async () => {
    ...
    server = setupServer(
      http.post<PathParams, CredentialSubject, VerifiableCredential<CredentialSubject>>('https://registrationnumber.notary.gaia-x.eu/v1/registrationNumberVC', async ({request, params, cookies}) => {
      ...
      http.post<PathParams, VerifiablePresentation<VerifiableCredential<CredentialSubject>>, VerifiableCredential<CredentialSubject>>('https://compliance.gaia-x.eu/development/api/credential-offers', async ({request, params, cookies}) => {
        ...
        return HttpResponse.json<VerifiableCredential<CredentialSubject>>({
          ...
          credentialSubject: vcs.map(credential => {
            return {
              ...
              "gx:version": "22.10",
              "gx:type": toArray(credential.credentialSubject)[0]['type'] || credential.type[0] || 'unknown'
            }
          }),
          proof: {
            type: 'JsonWebSignature2020',
  ...
  describe("Credentials CRUD", () => {
    ...
    it("Issue Credential", async () => {
      ...
      await expect(credentialsService.issueCredential({
        context: [],
        type: [],
    ...
    it("Request Gaia LRN", async () => {
      const credential = await credentialsService.requestLegalRegistrationNumberCredential(plainToInstance(LegalRegistrationNumberRequest, {
        vcId: `${didId}#LRN`,
    ...
    it("Request Gaia Compliance", async () => {
      const credential = await credentialsService.requestComplianceCredential(plainToInstance(ComplianceRequest ,{
        ...
        credentials: (await credentialsService.getCredentials()).map(c => c.credential)
      }), undefined)
      expect(credential).toBeDefined();
      expect(credential.id).toBe(`${didId}#LRN`)
    ...
    it("Delete credential", async () => {
      ...
      await expect(credentialsService.getCredential(`${didId}#test-credential`)).rejects.toThrow("can't be found");
      await expect(credentialsService.deleteCredential(`${didId}#test-credential`)).rejects.toThrow("can't be found");
    })
  })
})