...
export class PresentationController {
  ...
  @Get()
  async createPresentation(@Query('credentialId') credentialId: string, @Query('audience') audience: string | undefined, @Query('asJwt', new DefaultValuePipe(false), ParseBoolPipe) asJwt: boolean, @Query('unwrap', new DefaultValuePipe(true), ParseBoolPipe) unwrap: boolean): Promise<VerifiablePresentationJsonLd | VerifiablePresentationJwt> {
    if (asJwt) {
      if (!audience) {
        throw new AppError(`Audience parameter required for JWT-based presentations`, HttpStatus.BAD_REQUEST);
      }
      return this.presentationService.createVerifiablePresentationJwt(credentialId, audience, unwrap);
    } else {
      return this.presentationService.createVerifiablePresentationJsonLd(credentialId, unwrap);
    }
  }
