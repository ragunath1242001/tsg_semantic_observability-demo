...
import { ClientInfo, ClientSignup, ResetPassword } from "../model/clients.dto.js";
...
export class AuthController {
  ...
  async refresh(@Request() req: ExpressRequest) {
    ...
    return this.clientsService.validateRefreshToken(user.sub, user.refreshToken);
  }
  ...
  @Post('verify')
  @HttpCode(HttpStatus.OK)
  async verify(@Query('clientId') clientId: string, @Query('code') code: string): Promise<void> {
