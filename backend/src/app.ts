...
async function bootstrap() {
  const logLevelConfig = process.env['LOG_LEVEL']?.toLowerCase() || (process.env.NODE_ENV === 'production' ? 'info' : 'debug');
  let logLevel: 'log' | 'debug' | 'verbose'
  switch(logLevelConfig) {
    case 'info':
    case 'log': logLevel = 'log'; break;
    case 'debug': logLevel = 'debug'; break;
    case 'verbose': logLevel = 'verbose'; break;
  ...
  if (process.env['EMBEDDED_FRONTEND']) {
    app.setGlobalPrefix('api', {
      exclude: ['.well-known/did.json', 'context/(.*)', 'health']
    })
  }
